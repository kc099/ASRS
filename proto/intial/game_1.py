    ...
    def animate(self):
        ...
        try:
            if self.operation_mode == 'storing_moving':
                logger.debug(f"ANIMATE: storing_moving - Trolley at ({self.trolley_row}, {self.trolley_col}), Path remaining: {len(self.trolley_path)}")
                if self.trolley_path:
                    next_row, next_col = self.trolley_path.pop(0)
                    self.trolley_row = next_row
                    self.trolley_col = next_col
                    logger.debug(f"Trolley moved to ({next_row}, {next_col})")
                    self.update_grid_display()

                    if not self.trolley_path:
                        logger.info("Trolley reached storage location, starting placement")
                        self.operation_mode = 'storing_placing'
                        self.animation_cell_index = 0

                        self.animation_cells = []
                        start_row, start_col = self.pending_position
                        logger.debug(f"Placing box at ({start_row}, {start_col}) - Size: {self.pending_box.width}x{self.pending_box.length}")
                        ...
                        logger.debug(f"Animation cells: {len(self.animation_cells)} cells")
                        self.path_visualization.clear()
                        self.status_label.setText(f"📦 Placing...")
                        self.update_grid_display()

            elif self.operation_mode == 'storing_placing':
                logger.debug(f"ANIMATE: storing_placing - Cell index: {self.animation_cell_index}/{len(self.animation_cells)}")
                if self.animation_cell_index < len(self.animation_cells):
                    row, col = self.animation_cells[self.animation_cell_index]
                    logger.debug(f"Placing animation at cell ({row}, {col})")
                    item = self.table.item(row, col)
                    item.setBackground(PLACING_COLOR)
                    item.setText("📦")
                    self.animation_cell_index += 1
                else:
                    logger.info("Box placement animation complete, placing box in rack")
                    self.rack.place_box(self.pending_box, self.pending_position[0], self.pending_position[1])
                    logger.info(f"Box placed in rack with ID: {self.pending_box.box_id}")

                    db_box_id = add_box_to_db(self.pending_model_id)
                    logger.info(f"Box added to database with ID: {db_box_id}")

                    self.operation_mode = 'returning'
                    logger.debug(f"Calculating return path from ({self.trolley_row}, {self.trolley_col}) to origin")
                    self.trolley_path = a_star_pathfinding(self.rack.grid,
                                                          (self.trolley_row, self.trolley_col),
                                                          (ORIGIN_ROW, ORIGIN_COL))
                    logger.info(f"Return path calculated - {len(self.trolley_path)} steps")
                    self.status_label.setText(f"🔄 Returning...")
                    self.update_grid_display()

            elif self.operation_mode == 'retrieving_moving':
                logger.debug(f"ANIMATE: retrieving_moving - Trolley at ({self.trolley_row}, {self.trolley_col}), Path remaining: {len(self.trolley_path)}")
                if self.trolley_path:
                    next_row, next_col = self.trolley_path.pop(0)
                    self.trolley_row = next_row
                    self.trolley_col = next_col
                    ...
                    if not self.trolley_path:
                        logger.info(f"Trolley reached box {self.retrieving_box_id}, starting pickup")
                        self.operation_mode = 'retrieving_picking'
                        self.animation_cell_index = 0

                        box = self.rack.boxes[self.retrieving_box_id]
                        start_row, start_col = self.pending_position
                        logger.debug(f"Picking box at ({start_row}, {start_col}) - Size: {box.width}x{box.length}")
                        self.animation_cells = []
                ...
                logger.debug(f"ANIMATE: retrieving_picking - Cell index: {self.animation_cell_index}/{len(self.animation_cells)}")
                if self.animation_cell_index < len(self.animation_cells):
                    row, col = self.animation_cells[self.animation_cell_index]
                    logger.debug(f"Picking animation at cell ({row}, {col})")
                    item = self.table.item(row, col)
                    item.setBackground(RETRIEVING_COLOR)
                    item.setText("⬆️")
                    self.animation_cell_index += 1
                else:
                    logger.info(f"Box pickup animation complete, removing box {self.retrieving_box_id} from rack")
                    self.rack.remove_box(self.retrieving_box_id)
                    logger.info(f"Box {self.retrieving_box_id} removed from rack")
                ...
                logger.debug(f"ANIMATE: returning - Trolley at ({self.trolley_row}, {self.trolley_col}), Path remaining: {len(self.trolley_path)}")
                if self.trolley_path:
                    next_row, next_col = self.trolley_path.pop(0)
                    self.trolley_row = next_row
                    self.trolley_col = next_col
                    ...
                    if not self.trolley_path:
                        logger.info("Trolley returned to origin, completing operation")
                        self.animation_timer.stop()
                        self.is_animating = False

                        if hasattr(self, 'pending_box') and self.pending_box:
                            logger.info(f"Storage operation complete - Box ID: {self.pending_box.box_id}")
                            ...
                            logger.info(f"Retrieval operation complete - Box ID: {self.retrieving_box_id}")
                        ...
                        logger.debug(f"Updating maintenance cycles - Distance: {self.distance_traveled}")
